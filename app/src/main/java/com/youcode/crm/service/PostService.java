...
import com.youcode.crm.mapper.PostMapper;
...
public class PostService {
     ...
     */
    public PostDTO addNewPost(PostDTO post, Principal principal) {
        Post mappedPost = postMapper.mapPostDtoToPost(post);
        Employee employee = employeeRepository.findByEmail(principal.getName())
                .orElseThrow(() -> new IllegalStateException("Employee not found"));

        mappedPost.setAuthor(employee);
        mappedPost.setCreatedAt(LocalDateTime.now());
        return postMapper.mapPostToDTO(postRepository.save(mappedPost));
    }
     ...
     */
    @Transactional
    public PostDTO editPostContent(PostDTO post, Principal principal){
        Post mappedPost = postMapper.mapPostDtoToPost(post);
        Post editedPost = postRepository.findById(mappedPost.getId()).orElseThrow(() ->
                new ResponseStatusException(HttpStatus.NOT_FOUND, "Post does not exist"));
        if(isAuthorOfPost(editedPost, principal)) {
            editedPost.setTitle(mappedPost.getTitle());
            editedPost.setContent(mappedPost.getContent());
        } else {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, "You are not the author of this post");
        }
